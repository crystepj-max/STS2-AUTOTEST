...
def _resolve_project_base_dir(project: str | None) -> Path | None:
    """委托 core.workspace.resolve_project_base_dir（worker 入口共用）。"""
    from sts2_autotest.core.workspace import resolve_project_base_dir

    return resolve_project_base_dir(project)


def _create_adapter(adapter_type: str, project: str | None = None) -> GameAdapterProtocol:
    """委托 core.runtime_factory.create_adapter_from_env（worker 入口共用）。"""
    from sts2_autotest.core.runtime_factory import create_adapter_from_env

    return cast(
        GameAdapterProtocol, create_adapter_from_env(adapter_type, project=project)
    )
...
def _load_workspace() -> Any | None:
    """委托 core.workspace.load_default_workspace（worker 入口共用）。"""
    from sts2_autotest.core.workspace import load_default_workspace

    return load_default_workspace()
...
def run_cmd(args: Any) -> int:
    ...
    if getattr(args, "journey", None) or getattr(args, "target_scene", None):
        # 旅程类 worker：排队、阶段位、执行与收口都在任务运行时 interface 之后。
        evidence_mode = getattr(args, "evidence", None)
        if evidence_mode:
            os.environ["STS2_AUTOTEST_EVIDENCE"] = str(evidence_mode)
        request = _journey_request_from_args(args)
        adapter_type = args.adapter or ("agent" if _is_agent_default() else "cli")
        project = getattr(args, "project", None)
        return run_journey_worker(
            lambda: _create_adapter(adapter_type, project=project),
            request,
            internal_run_id,
        )
    from sts2_autotest.core.run_service import (
...
def _journey_request_from_args(args: Any) -> JourneyRequest:
    """把 run 旗标翻译为旅程请求；真实入口固定开启预检门禁。"""
...
def _run_cmd_foreground(args: Any) -> int:
    ...
    if getattr(args, "journey", None) or getattr(args, "target_scene", None):
        return JourneyExecutor(
            adapter,
            _journey_request_from_args(args),
