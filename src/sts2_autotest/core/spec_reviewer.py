...
_SUPPORTED_STEP_PATTERNS: list[re.Pattern[str]] = [
    ...
    for pattern in (
        ...
        # 角色选择：任意 ASCII 标识符或原游戏角色中文名；MOD 角色别名由项目配置提供
        r"选择\s*([A-Za-z][A-Za-z0-9_-]*|战士|铁甲战士)|开始冒险",
        r"开局事件.*第\s*\d+\s*个选项|选择涅奥祝福|推进事件对话|点击\s*Proceed|点击继续前进|选择\s*Proceed",
        r"地图节点.*\(\s*\d+\s*,\s*\d+\s*\)|选择首个可走地图节点|选择第一个可走地图节点|选择首个战斗节点|选择首个普通战斗节点|选择首个营火节点|选择营火选项\s*\d+|进入首次战斗|进入首场战斗",
        r"收取奖励并继续",
