...
- `project`：项目标识或项目目录。项目标识经本地 workspace 配置（`sts2-autotest.yaml`，本地设置、不随平台分发）中的 manifest 指针解析；项目目录直接指向包含 `sts2-mod.yaml` 的项目根目录。任务携带 `project` 时，平台按该项目自己的 `project_extension` 配置执行卡牌前缀映射、种子命令模板与角色别名（含编译与连接恢复路径）；未携带时平台保持中性，不内置任何项目规则。
