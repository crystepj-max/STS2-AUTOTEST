...
- **政策变更 PR（issue #80）**：Agent 可以创建独立政策 PR、推送代码、等待或触发 CI；**禁止**对政策变更执行 `gh pr merge`（含 `--admin`）。政策 PR 必须由维护者本人点 Merge。CODEOWNERS 仍标识负责人，但 ruleset 不再要求 code owner review。
