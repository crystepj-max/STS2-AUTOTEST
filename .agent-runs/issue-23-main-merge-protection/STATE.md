...
1. PR #33 审查线程按最终提交同步：Bot 共 12 条线程（P1×2：macOS 缺陷复验/超时异常类型；P2×10：超时、bash 跳过、LC_ALL、UTF-8 解码、进程树清理、NoSuchProcess 容错、清理等待超时、文档表述等），
   全部已回复并标记解决（required_review_thread_resolution=true 下合并前须确认无未解决线程）。
