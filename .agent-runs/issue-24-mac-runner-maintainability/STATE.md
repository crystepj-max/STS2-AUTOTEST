...
- 阶段：T1 取证已归档；T2/T3(脚本)/T4/T5 开发完成；S4 返工完成；
  ...
  T3 探针已部署、连续采集中（完整四类探针首采 2026-08-14 16:36 UTC，2026-08-21 16:36 满 7 天，**满 7 天前不视为完成**）；
...
2. **T3 部署：✅ 已部署，⏳ 采集中（未完成）** → evidence/deployment-t3-20260814.md
   ...
   `~/.sts2-runner-probe/`；**连续采集满 7 天（2026-08-21 16:36 UTC）才算 T3 完成**）。
...
4. **T7 收口：⏳ 待 8/22**——定时任务已设（**2026-08-22 01:00 本地 CST
   = 17:00 UTC，晚于 16:36 UTC 满 7 天门禁**，持久化 CronCreate），检视
   T2/T3/T6/T7 结果后决策是否关闭 Issue。本地时区 CST(UTC+8) 已换算，
   定时任务在满 7 天门禁之后触发，不会提前误判。

## 下一步（关闭前置条件）

1. T3 数据连续采集至 2026-08-21 16:36 UTC 满 7 天（未满不视为完成）。
2. **T6 独立演练**：另请**非原排障人员**按手册第 7 节完成演练并落盘证据
   （`runbook_drill_passed` 门禁；当前原排障者演练不满足）。
3. 8/22 01:00 本地（= 17:00 UTC）定时任务触发：检视 T2（已完成）/T3（满
   7 天）/T6（独立演练）/T7 结果。
